            ...
            def worker(r):
                try:
                    ...
                    if is_machine(r):
                        ...
                        r.run_command("if [ ! -f /etc/nix/nix.conf ]; then mkdir -p /etc/nix && echo 'binary-caches = http://nixos-cache.benaco.com/ http://cache.nixos.org/\\nbuild-max-jobs = 5' > /etc/nix/nix.conf; else cp /etc/nix/nix.conf /tmp/nix.conf && perl -p -i -e 's@^binary-caches.*$@binary-caches = http://nixos-cache.benaco.com/ http://cache.nixos.org/@g' /tmp/nix.conf && perl -p -i -e 's@^build-max-jobs.*$@build-max-jobs = 5@g' /tmp/nix.conf && perl -p -i -e 's@^binary-cache-public-keys.*$@binary-cache-public-keys = benaco-nix-binary-cache-signing-key:TiYm+ouXxHq1yu3YyjVBseCwjj8uOE8AWPF1wDo5JbI= cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY=@g' /tmp/nix.conf && mv /tmp/nix.conf /etc/nix/nix.conf; fi")
