...
# may still occur.
containers_process_global_nix_build_lock = threading.Lock()
...
class ContainerState(MachineState):
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        if self.vm_id is None:
            ...
            self.log("Waiting to lock containers_process_global_nix_build_lock to work around nixops bug #889")
            containers_process_global_nix_build_lock.acquire()
            self.log("Locked containers_process_global_nix_build_lock to work around nixops bug #889")
            path = subprocess.check_output(
                ["nix-build", "<nixpkgs/nixos>", "-A", "system",
                 "-I", "nixos-config={0}".format(expr_file)]
                + self.depl._nix_path_flags()).rstrip()
            containers_process_global_nix_build_lock.release()
            self.log("Unlocked containers_process_global_nix_build_lock to work around nixops bug #889")
